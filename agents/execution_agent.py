...
BYBIT_BASE   = "https://api-testnet.bybit.com"  # testnet
...
def _calculate_qty(balance: float, risk_pct: float, entry: float, sl: float) -> float:
...
def build_trade_plan(symbol: str, payload: dict) -> dict:
    ...
    if hasattr(bias, "direction"):
        direction = bias.direction
    else:
        direction = bias.get("direction", "NEUTRAL")

    if hasattr(trigger, "entry"):
        entry = trigger.entry
        sl    = trigger.sl
        tp1   = trigger.tp1
        tp2   = trigger.tp2
    ...
    if hasattr(score, "confidence"):
        confidence = score.confidence
        grade      = score.grade
        risk_label = score.risk
    else:
